...
import style from "./BaiTapDanhSachPhim.module.css";
...
export default class BaiTapRenderDanhSachPhim extends Component {
  renderPhimWithMap = () => {
    const arrJSX = mangPhim.map((phim, index) => {
      return (
        <div className="col-4" key={index}>
          <div class="card text-dark">
            {/* <img
              class={`card-img-top ${style["hinhAnhPhim"]}`}
              src={phim.hinhAnh}
              alt=""
            /> */}
            ...
            />
            <div class="card-body">
              {/* nếu nội dung dài quá cho dấu ... bằng hàm substr() */}
              <h4 class="card-title" style={{ height: "60px" }}>
                {phim.tenPhim.length > 50 ? (
                  <span>{phim.tenPhim.substr(0, 50)}...</span>
                ) : (
              ...
              <p class="card-text">
                {phim.moTa.length > 50 ? (
                  <span>{phim.moTa.substr(0, 50)}...</span>
                ) : (
                  <span>{phim.moTa}</span>
                )}
              </p>
  ...
  render() {
    return (
      <div className="container">
        ...
        <div className="row">{this.renderPhimWithMap()}</div>
      </div>
    );
  }
}
